...
const { Scene, Frustum, Matrix4, Vector2, Vector3, Vector4, Color, DoubleSide, BackSide, FrontSide, SRGBColorSpace, NoToneMapping } = THREE;
import fs from 'fs';
...
class Renderer {
	...
	async init() {
		...
		const shadersDirectory = path.join('C:', 'Users', 'vande', 'Documents', 'Code', 'LutExplorer', 'shaders');
		fs.readdir(shadersDirectory, (err, files) => {
   		 if (err) throw err;
 		   for (const file of files) {
			const filePath = path.join(shadersDirectory, file);
...
// Code de rendu 3D basique avec Three.js
const THREE = require('three');
...
// Code pour le chargement et l'affichage d'images
const imageInput = document.getElementById('image-input');
if (imageInput) { // Vérifiez si l'élément existe pour éviter les erreurs
